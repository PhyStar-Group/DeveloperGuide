...
\section{CartBoxFFT三方向周期傅里叶求解器}
\subsection{空间离散}
此类求解器需要流场在三个方向均满足周期边界条件,且网格均匀分布。三个方向的网格数记为$N_x, N_y$和$N_z$。此时，在三个方向均采用伪谱方法进行空间离散。
\paragraph*{傅里叶变换}
任意物理空间的物理量$a$可通过三维快速傅里叶变换转到谱空间，记为$\hat{a}$, 即
\begin{equation}
	\hat{a}(p,q,r)=\sum_{k=0}^{N_z-1}\sum_{j=0}^{N_y-1}\sum_{i=0}^{N_x-1} a(i,j,k)e^{-\text{i}px(i)\frac{2\pi}{L_x}-\text{i}qy(j)\frac{2\pi}{L_y}-\text{i}rz(k)\frac{2\pi}{L_z}}
\end{equation}
可记为$\hat{a}=\mathcal{F}(a)$。其中$p,q,r$为谱空间三个方向的谱波数，其波数范围分别为$[-N_x/2+1,N_x/2]$, $[-N_y/2+1,N_y/2]$和$[-N_z/2+1,N_z/2]$。同理，谱空间变量$\hat{a}$可通过傅里叶逆变换转换到物理空间$a$, 即
\begin{equation}
	a(i,j,k)=\frac{1}{N_xN_yN_z}\sum_{r=-N_z/2+1}^{N_z/2}\sum_{q=-N_y/2+1}^{N_y/2}\sum_{p=-N_x/2+1}^{N_x/2} \hat{a}(p,q,r)e^{\text{i}px(i)\frac{2\pi}{L_x}+\text{i}qy(j)\frac{2\pi}{L_y}+\text{i}rz(k)\frac{2\pi}{L_z}}
\end{equation}
记为$a=\mathcal{F}^{-1}(\hat{a})$

\paragraph*{伪谱法与消除混淆误差}
对于非线性项，例如$a\cdot b$, 需要先将$a$与$b$在谱空间将波数范围扩充至$[-3N_x/2+1,3N_x/2]$, $[-3N_y/2+1,3N_y/2]$和$[-3N_z/2+1,3N_z/2]$，再通过傅里叶逆变换至物理空间，记为$\tilde{a}$ 和$\tilde{b}$。在物理空间进行乘法积后，对$\tilde{a}\cdot\tilde{b}$进行傅里叶变换，并将波数缩减到$[-N_x/2+1,N_x/2]$, $[-N_y/2+1,N_y/2]$和$[-N_z/2+1,N_z/2]$，而之外的波数被赋零截断。这种伪谱方法的操作记为$\mathcal{F}_{quasi}(\cdot)$。对应的原理示意图见\obs{FIG}。对混淆误差的消除步骤已经内置于TFFT3D程序包，可以通过设置网格数量直接实现混淆误差的消除。
...
\begin{equation}
	\frac{\hat u_i^{n+1}-\hat u_i^n}{\Delta t}=\frac{3}{2}T(u_i^n)-\frac{1}{2}T(u_i^{n-1})-\frac{1}{\rho}\frac{\partial \hat p^{n+1}}{ \partial x_i}
\end{equation}
其中
\begin{equation}
	T(u_i^n)=-\frac{\partial \mathcal{F}_{quasi}(u_i^n u_j^n)}{\partial x_j}+\nu\frac{\partial^2 \hat u_i^n}{\partial x_j \partial x_j}+\hat{f}_i^n
\end{equation}
求解步骤：\\
...
\end{equation}
第二步: 求解压力步$p^{n+1}$,
\begin{equation}\label{eq:fluid-pressure-step}
\nabla^2p^{n+1}=\frac{\rho \nabla\cdot u^*}{\Delta t}
\end{equation}
第三步：求解速度$u_i^{n+1}$
\begin{equation}\label{eq:fluid-correct-step}
u_i^{n+1}-u^*=-\Delta t\frac{\partial p^{n+1}}{\partial x_i}
\end{equation}

值得注意的是，公式~(\ref{eq:fluid-pressure-step}) 和(\ref{eq:fluid-correct-step})可使得流场无散。可证明，将式(\ref{eq:fluid-correct-step})两侧同时求散度，若$\ve \nabla \cdot \ve u_i^{n+1}=0$， 可得到式~(\ref{eq:fluid-pressure-step})。
...
v_s(i,j,k)&=\frac{\alpha}{\delta}\sqrt{e}y^+e^{-\frac{1}{\delta^2}{y^+}^2}\sin{\frac{L_x^+}{l_x^+}\frac{2\pi i}{N_x}}.\\
w_s(i,j,k)&=0;\\
\end{split}
\end{equation}
